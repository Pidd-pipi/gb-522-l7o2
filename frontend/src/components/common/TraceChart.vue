...
const props = withDefaults(defineProps<{
  ...
  launchOffsetM?: number
  events?: EventMarker[]
  noiseFloor?: number
  height?: number
}>(), { refractiveIndex: 1.468, launchOffsetM: 0, events: () => [], noiseFloor: 0, height: 360 })
...
function distanceAt(index: number) {
  const raw = 299792458 * index * props.sampleIntervalNs * 1e-9 / (2 * props.refractiveIndex)
  return Number((raw - props.launchOffsetM).toFixed(2))
}
...
watch(() => [props.points, props.events, props.noiseFloor, props.refractiveIndex, props.launchOffsetM], render, { deep: true })
</script>
