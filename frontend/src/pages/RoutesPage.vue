...
const offsetOnly = computed(() => auth.user.value?.role === 'reviewer')
...
async function update() { if (!selected.value) return; busy.value = true; try { const payload = offsetOnly.value ? { launch_offset_m: editForm.launch_offset_m } : { ...editForm }; const updated = await store.update(selected.value.id, payload); selected.value = updated; editOpen.value = false; ElMessage.success('线路档案已更新') } finally { busy.value = false } }
onMounted(search)
</script>
...
<template>
  ...
  <section class="content-band">
    ...
    <div class="data-surface">
      <el-table v-loading="store.loading" :data="store.items" row-key="id" @row-dblclick="inspect">
        ...
        <el-table-column prop="launch_offset_m" label="发射端偏移" width="110"><template #default="scope">{{ scope.row.launch_offset_m.toLocaleString() }} m</template></el-table-column>
  ...
  <el-drawer v-model="detailOpen" :title="selected ? `${selected.route_code} / ${selected.name}` : '线路详情'" size="min(860px, 94vw)">
    <template #header><div class="drawer-title"><span>{{ selected ? `${selected.route_code} / ${selected.name}` : '线路详情' }}</span><el-button v-if="selected && (auth.canAnalyze() || auth.canReview())" size="small" @click="openEdit"><Pencil :size="14" />编辑档案</el-button></div></template>
    <div v-if="selected" class="route-facts"><span><small>长度</small><strong>{{ selected.length_m.toLocaleString() }} m</strong></span><span><small>折射率</small><strong>{{ selected.refractive_index }}</strong></span><span><small>发射端偏移</small><strong>{{ selected.launch_offset_m.toLocaleString() }} m</strong></span><span><small>基线轨迹</small><strong>{{ selected.baseline_trace_id ? `#${selected.baseline_trace_id}` : '未设置' }}</strong></span></div>
    <TraceChart v-if="preview && selected" :points="preview.trace.processed_points.length ? preview.trace.processed_points : preview.trace.points" :sample-interval-ns="preview.trace.sample_interval_ns" :refractive-index="preview.trace.refractive_index_snapshot ?? selected.refractive_index" :launch-offset="preview.trace.launch_offset_snapshot_m ?? 0" :events="preview.events" :noise-floor="preview.trace.noise_floor_db" :height="300" />
  ...
  <el-dialog v-model="editOpen" :title="offsetOnly ? '编辑发射端偏移' : '编辑线路档案'" width="min(620px, calc(100vw - 28px))">
    <el-alert v-if="offsetOnly" type="info" :closable="false" title="复核员仅可修正发射端尾纤偏移，其余线路参数由分析员维护。" style="margin-bottom:14px" />
    <el-form label-position="top"><div class="form-grid"><template v-if="!offsetOnly"><el-form-item label="线路名称"><el-input v-model="editForm.name" /></el-form-item><el-form-item label="线路长度（m）"><el-input-number v-model="editForm.length_m" :min="1" :max="500000" style="width:100%" /></el-form-item><el-form-item label="折射率"><el-input-number v-model="editForm.refractive_index" :min="1.3" :max="1.7" :precision="4" :step="0.001" style="width:100%" /></el-form-item><el-form-item label="发射端连接器"><el-input v-model="editForm.launch_connector" /></el-form-item><el-form-item label="线路状态"><el-select v-model="editForm.route_status" style="width:100%"><el-option label="在线" value="active" /><el-option label="维护" value="maintenance" /><el-option label="退役" value="retired" /></el-select></el-form-item></template><el-form-item label="发射端偏移（m）"><el-input-number v-model="editForm.launch_offset_m" :min="0" :max="10000" :precision="2" style="width:100%" /></el-form-item></div></el-form>
    <template #footer><el-button @click="editOpen=false">取消</el-button><el-button type="primary" :loading="busy" :disabled="!offsetOnly && (!editForm.name || !editForm.launch_connector)" @click="update">保存变更</el-button></template>
  </el-dialog>
</template>
