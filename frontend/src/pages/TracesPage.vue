...
const segmentParams = computed(() => {
  const trace = traces.selected?.trace
  return {
    length: trace?.route_length_snapshot_m ?? selectedRoute.value?.length_m ?? 0,
    refractive: trace?.refractive_index_snapshot ?? selectedRoute.value?.refractive_index ?? 1.468,
    offset: trace?.launch_offset_snapshot_m ?? 0,
  }
})
const hasSnapshot = computed(() => traces.selected?.trace.launch_offset_snapshot_m != null)
  ...
  <section class="content-band trace-workspace">
    ...
    <div class="analysis-stage">
      <template v-if="traces.selected">
        ...
        <div class="segment-params"><span><small>线路段长度（快照）</small><strong>{{ segmentParams.length.toLocaleString() }} m</strong></span><span><small>折射率（快照）</small><strong>{{ segmentParams.refractive }}</strong></span><span><small>发射端偏移（快照）</small><strong>{{ segmentParams.offset.toLocaleString() }} m</strong></span><em v-if="!hasSnapshot">旧轨迹未保存偏移，检测按 0 处理</em></div>
        <TraceChart :points="traces.selected.trace.processed_points.length ? traces.selected.trace.processed_points : traces.selected.trace.points" :sample-interval-ns="traces.selected.trace.sample_interval_ns" :refractive-index="segmentParams.refractive" :launch-offset="segmentParams.offset" :events="traces.selected.events" :noise-floor="traces.selected.trace.noise_floor_db" />
